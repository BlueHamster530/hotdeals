...
# 일부 커뮤니티(클리앙·다모앙 등)는 비-브라우저 UA를 403으로 막는다.
# 그런 소스만 extra_headers로 브라우저 UA를 덮어쓴다(기본은 정직한 봇 UA 유지).
BROWSER_HEADERS = {
    "User-Agent": (
...
class Source(abc.ABC):
    ...
    name: str
    kind: str          # "rss" | "html"
    extra_headers: dict[str, str] = {}  # 요청 헤더 오버라이드(예: 브라우저 UA). 기본 없음.
...
async def enrich_og_thumbnails(
    ...
    """썸네일이 없는 딜에 한해 글페이지 og:image를 채운다(RSS에 이미지가 없는 소스용).
